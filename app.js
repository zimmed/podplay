...
// Session setup
app.use(session);
// Middleware function to extend read-only session data to
//      socket objects. (socket.request.session)
//      NOTE: This is accessed from Router Handler functions
//          by use of `this.request.session`.
...
// Set interval to continue to update every 2 hours.
var top100timer = setInterval(badcache.update_top100, UPDATE_TOP100_INTERVAL);

// [DEBUG] Print out all registered users to console
//      when server starts.
if (app.get('env') === 'development') {
