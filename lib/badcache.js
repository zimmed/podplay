...
var Cache = {
    ...
    top100: [],
    top100cat: {},
    aggregate : function (subscriptions) {
        var i, g, c = 0, fcasts = {}, pcasts = {};
        if (subscriptions) {
            for (g in subscriptions) {
                i = Podcasts.getGenreId(g);
                Podcasts.getPodcasts(subscriptions[g], function () {
                }, function (results) {
                    fcasts[i] = results;
                });
            }
        ...
        Podcasts.getPodcasts(this.top100.slice(0, 10), function () {
        }, function (results) {
            pcasts['0'] = results;
        });
        for (i in this.top100cat) {
            Podcasts.getPodcasts(this.top100cat[i], function () {
            }, function (results) {
                pcasts[i] = results;
            });
        }
        while (pcasts.length != this.top100cat.length) {
            console.log(pcasts.length + " != " + this.top100cat.length);
        }
        console.log(pcasts['0']);
        return [pcasts, fcasts];
    },
