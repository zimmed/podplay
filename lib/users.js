...
module.exports = {
    ...
    delSubscription : function (user, id, failure, success) {
        this.User.findById(user._id, function (err, doc) {
            if (err) {
                ...
                function (podcast) {
                    ...
                    var index = doc.subscriptions[podcast.genre].indexOf(podcast.id);
                    if (index == -1) dbError(false, failure);
                    doc.subscriptions[podcast.genre].splice(index, 1);
                    // Update database.
        ...
        this.User.findById(user._id, function (err, doc) {
            if (err) {
                ...
                podcasts.getPodcast(id, function (error, msg) {
                    ...
                    function (podcast) {
                        // Successfully pulled podcast data from cache.
                        if (!doc.subscriptions[podcast.genre])
                            doc.subscriptions[podcast.genre] = [];
                        doc.subscriptions[podcast.genre].push(podcast._id);
                        saveDoc(doc, failure, success);
                    });
            }
        });
    ...
    validateUser : function (name, password, failure, success) {
        ...
        this.User.findOne({'name': name}, function (err, user) {
            if (err) {
                error.err = err;
    ...
    registerUser : function (name, email, password, failure, success) {
        ...
        this.User.count({'name':  name}, function (err, c) {
            if (err) {
                // Database error
                dbErr(err, failure);
            }
                ...
                this.User.count({'email': email}, function (err, c) {
                    if (err) {
                        // Database error
                        dbErr(err, failure);
                    }
