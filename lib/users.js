...
var users = {
    ...
    isFavorited : function (user, id) {
        console.log(JSON.stringify({id: id, user: user}));
        if (!user.subscriptions || !user.subscriptions[0]) return false;
        return (user.subscriptions[0].indexOf(id) !== -1);
    },
        ...
        users.User.findById(user._id, function (err, doc) {
            if (err || !doc) {
                ...
                function (podcast) {
                    ...
                    if (users.isFavorited(doc, id)) {
                        var index = doc.subscriptions[g].indexOf(podcast.id),
                            index0 = doc.subscriptions[0].indexOf(podcast.id);
